...
declare function log:ignore($id,$host) as xs:boolean  {
   exists(doc(concat("/db/apps/",$id,"/logs/ignore.xml"))//host[starts-with($host,.)])
};
...
return 
    ...
    then ()
         ...
         let $url := concat("http://freegeoip.net/xml/",$ip)
...
declare function log:log-request($id  as xs:string) {
  log:log-request($id,(),())
};
...
return 
    if (log:ignore($id, $logrecord/@host))
    then ()
    else update insert $logrecord into $log
};
