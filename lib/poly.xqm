...
declare function poly:geojson-coordinates-to-polygon($points) {
   element polygon {
       for $p in $points
       let $lat-long := tokenize(normalize-space($p),",")
       return element point {attribute latitude {round-half-to-even($lat-long[2],6)}, attribute longitude {round-half-to-even($lat-long[1],6)}}
   }
};
