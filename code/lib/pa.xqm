...
declare variable $pa:recommendations := ("","OBJECT","SUPPORT","UNDER CONSIDERATION","NEUTRAL");
declare variable $pa:group := doc(concat($pa:dbroot,"/ref/btf-conf.xml"))/group;
declare variable $pa:applications :=concat($pa:dbroot,"applications");
declare variable $pa:date-format := "[D01]-[M01]-[Y03]";
declare variable $pa:update-job := "btf-refresh";
...
declare function pa:get($keyval,$page) {
   ...
   let $statusCode := $page/@statusCode
...
let $comment-list := 
        ...
        let $description := $comment/h:td[5]
        let $address := if (contains($description,"-")) then substring-before($description,"-") else substring-before($description,"(")
        let $support := contains($description,"SUPPORT")
        let $object := contains($description,"OBJECT")
...
let $page1 := $converted-doc//h:div[@class="page"][1]
let $customer-details := $page1/h:p[starts-with(.,"Customer Details")]
let $customer-details := if (string($customer-details) != "" ) 
                         then $customer-details
                         else $page1/h:p[starts-with(.,"Name:")]
let $customer-details := if (exists($customer-details))
                        then $customer-details
                        else substring-before(substring-after($page1/h:p[starts-with(.,"Case officer")],"Customer details"),"Comments")
let $comment-details := $page1/h:p[starts-with(.,"Comment Details")]
let $comment-details := if (exists($comment-details)) 
                        then substring-after($comment-details  ,"Comment:") 
                        else if (exists($page1/h:p[starts-with(.,"Comments")]))
                        then ()
                        else substring-after($page1/h:p[starts-with(.,"Case officer")],"Comments")
                        
let $comment-rest := if (exists($page1/h:p[.="Comments"]))
                     then $page1/h:p[.="Comments"]/following-sibling::*
                     else $page1/h:p[starts-with(.,"Comment Details")]/following-sibling::*
return 
element submission {
   element date-stored {current-dateTime()},
   element comments {
           if ($comment-details != "") then element h:p {$comment-details} else (),
           $comment-rest,
    ...
    element customer {
         ...
         let $add := normalize-space(substring-after($customer-details,"Address:"))
         return if ($add = "*No Address Given") then () else element address {$add}
         }
 }
};
...
declare function pa:refresh-application($app) {
    ...
    let $update := if ($summary) then update replace $app/summary with $summary else ()
...
declare function pa:schedule-refresh() {
   let $interval := "0 0 22 * * ?" (: run at 10pm every day :)
...
return
  <div>  
    <form action="{$pa:root}application/{$keyval}/update" method="post">
      <table>
         ...
         <tr><th>Sitecode</th><td><input type="text" name="sitecode" value="{$local/sitecode}" size="20"/></td></tr>
         <tr><th>BTF position</th>
             <td> 
 ...
 declare function pa:update-application($keyval) {
     ...
     let $sitecode := request:get-parameter("sitecode",())
     let $local := 
          element local {
             ...
             if ($sitecode !="")
             then element sitecode {$sitecode}
             else ()
         }
...
declare function pa:rec-colour($text) {
     ...
     else if ($s="UNDER CONSIDERATION") then "orange"
   ...
   { string-join(
        ...
        let $position := if (exists($local/recommendation) and $local/recommendation != "")
                        then $local/recommendation/string()
                            ...
                            let $group-comment := $app/comments/comment[starts-with(address,$pa:group/response-address)]
                            return 
                                if($group-comment)
        ...
        let $icon :=       
          if ($position="SUPPORT")
          then "https://maps.google.com/mapfiles/ms/icons/green-dot.png"
          else if ($position="OBJECT")
          then "https://maps.google.com/mapfiles/ms/icons/red-dot.png"
          else if ($position="UNDER CONSIDERATION")
          then "https://maps.google.com/mapfiles/ms/icons/orange-dot.png"
          else if ($position="NEUTRAL")
          then "https://maps.google.com/mapfiles/ms/icons/blue-dot.png"
          else "https://maps.google.com/mapfiles/ms/icons/yellow-dot.png"
        return
...
<table class="sortable" >
 <tr><th>Reference</th><th width="8%">Validation date</th><th>Address</th><th>Proposal</th><th># Comments</th><th>BTF submission</th><th width="8%">BTF submission date</th><th>Status</th><th width="8%">Status date</th></tr>
 {for $app in $apps
  let $summary := $app/summary
  let $comments:= $app/comments
  let $status := $summary/status
  let $local:= $app/local
  let $group-comment := $comments/comment[starts-with(address,$pa:group/response-address)]
  ...
  return
   <tr > 
      <td><a href="{$pa:root}application/{$app/keyval}">{if ($summary/reference !="") then $summary/reference else $app/keyval/string()}</a></td>
...
declare function pa:show-comment($comment,$reveal) {
   <div>
       <h4><span>{if ($reveal) then <span>{$comment/customer/name/string()}&#160;{$comment/name/string()}&#160;{$comment/address/string()}&#160;</span>else ()}{if($comment/date) then <span> on {$comment/date/string()} </span> else ()} </span>
...
declare function pa:show-application($app) {
    ...
    let $articles :=  doc("data/articles.xml")//article[keyval=$keyval]
    return
     <div>
     <h2>Application Details</h2>
      <table class="left_col">
         ...
         {if ($sitecode)
         then <tr><th>Sitecode</th><td><a target="_blank"  class="external" href="https://bristoltrees.space/Tree/sitecode/{$sitecode}">{$sitecode/string()}</a></td></tr>
         else ()
        }
         ...
         then <tr><th>Last refreshed</th><td>{format-dateTime($summary/extraction_dateTime,$pa:date-format)} &#160;
               <span><a href="{$pa:root}application/{$keyval}/refresh">Refresh Data from BCC</a>&#160;
                     ...
                     <a href="https://bristoltrees.space/btfpa/xquery/geocode-application.xq?keyval={$keyval}">Geocode comments</a>
               </span> 
               </td>
              </tr>
          else ()
          }
        <tr><th>BCC Planning Portal</th><td><a class="external" target="_blank" href="{pa:bcc-link($keyval,"summary")}">BCC Planning Portal</a></td></tr>
        ...
        <tr><th>Comment analysis</th>
            <td>   {if ($comments//geocode)
                   then <a target="_blank" class="external" href="https://bristoltrees.space/btfpa/map-application.xq?keyval={$keyval}">Map</a>
                  else ()
                   }
                   &#160;
                   <a target="_blank" class="external" href="https://bristoltrees.space/btfpa/date-analysis.xq?keyval={$keyval}">Date of Submission</a>
            </td>
        </tr>
        {if ($articles)
        then <tr><td>BTF articles</td>
         ...
         <tr><th>Nearby Trees </th><td><a target="_blank"  class="external" href="https://bristoltrees.space/Tree/search/tree?range=200&amp;latitude={$latitude}&amp;longitude={$longitude}">Within 200m</a></td></tr>
       </table>
        <div  class="left_col">
             <h3>{$pa:group/abbrev/string()} response: 
         ...
         <div class="comment_col">
            <h3>Public Comments</h3>
              {if ($group-comment) 
              then pa:show-group-comment($pa:group,$group-comment)
              else ()
              }
              {for $comment in $comments/comment except $group-comment
               order by $comment/date descending
               return pa:show-comment($comment,$summary/decision="")
              }
         </div>
...
<div>
 ...
 <!--       <span><a class="button" href="{$pa:root}fullmap">Full Size Map</a> </span>  -->
       ...
       <span>UNDER CONSIDERATION <img src="http://maps.google.com/mapfiles/ms/icons/orange-dot.png"/></span>
 ...
 <div>    
    <h3>Site administration</h3>
     ...
     (<li>{pal:user()/string()} Logged in. <a href="{$pa:root}admin/logout">Logout</a></li>,
      ...
      <li><a href="{$pa:root}admin/create-form">Create new Application</a>
          &#160;<a target="_blank" href="https://planningonline.bristol.gov.uk/online-applications/search.do?action=simple&amp;searchType=Application">Search BCC applications</a>
      </li>,
     ...
     <li><a href="{$pa:root}admin/link-article-form">Link to BTF article</a></li>,
     <li><a href="{$pa:base}/trees/../btfpa/logs/log.xml">Raw Log</a></li> 
     )
     else
     (
...
<div>
    <div>These notes describe the admin functions on the BTF planning portal </div>
    <h3>Registering</h3>
    <ul>
    ...
    <h3>Editing our application data</h3>
      <ul>
        ...
        <li>The planning portal keeps and allows editing of data additional to that extracted from the Planning Portal. This includes:
            <ul>
                <li>The response of the BTF to this application.  This may be SUPPORT, OBJECT, NEUTRAL or undecided.</li>
    ...
    <h3>Linking</h3>
    <ul>
      <li>The page for an application has a url such as http://bristoltrees.space/Planning/application/OU2CFQDNG2A00  This is the link to use when referencing an application on the BTF web site or to use in email communication.</li>
    </ul>
    <h3>Known issues</h3>
    <ul>
...
<div>
    <p>Welcome to the Bristol Tree Forum Planning Portal, our innovative interactive tool presenting details
        of the main planning applications in our area lodged recently with Bristol City Council as
        the local planning authority. </p>
    <h3>Map</h3>
    <ul>
        <li>The Google map shows the boundaries of the Bristol area. </li>
    ...
    <ul>
        <li>This portal covers planning applications which have been noted by members of the Bristol Tree Forum. </li>
    ...
    <ul>
        ...
        <li>The data presented is drawn primarily from Bristol City Council’s <a target="_blank" class="external" href=" http://planningonline.bristol.gov.uk/online-applications/">searchable planning database</a> , with value-added content added by the BTF team. </li>
    ...
    <h3>Feedback</h3>
    <ul>
        <li>Comments and feedback on this portal are welcome - kit.wallace@gmail.com.</li>
    </ul>
    <h3> Disclaimer</h3>
    <ul>
        <li>Whilst we have confidence in the software and data used in the construction of this
            ...
            and are not liable for any errors or inaccuracies you may encounter. </li>
    </ul>
    <h3>Admin</h3>
    <ul>
        <li>
 ...
 declare function pa:analysis ($apps) {
   let $data :=
      ...
      let $position := if ($app/local/recommendation != "")
                        ...
                        else let $group-comment := $app/comments/comment[starts-with(address,$pa:group/response-address)]
                             return
                             if ($group-comment)
                             then if ($group-comment/support)
                                  then "SUPPORT"
...
<tr><th>Total</th>
 ...
 declare function pa:glossary() {
    let $terms := doc("/db/apps/btfpa/data/glossary.xml")//term
    return
    <div>
       <h2>Glosssary of Planning Terminology</h2>
       <div>
       This a work in progress to provide visitors to the portal with explanations of planning terminology and links to relevant documents.   All suggestions of suitable content welcome.
       </div>
       {for $term in $terms
 ...
 declare function pa:link-article-form() {
    <div>
      <form action="{$pa:root}admin/link-article" >
       Enter URL of BTF Article : <input type="text" size="70"  name="url"/>
 ...
 declare function pa:link-article(){
   let $articles := doc("/db/apps/btfpa/data/articles.xml")/articles
   let $url := request:get-parameter("url",())
   let $doc := httpclient:get(xs:anyURI($url),false(),())
   let $title := $doc//head/title
   let $links := 
       $doc//a/@href[starts-with(.,"https://bristoltrees.space/Planning/application/")]
...
declare function pa:events($keyval) {
    doc("events.xml")//event[keyval=$keyval]
};
 
declare function pa:cum($seq,$total) {
  if (empty($seq))
  then $total
  else let $tot := $total+$seq[1]
       return ($tot,pa:cum(subsequence($seq,2),$tot) )
};
