...
let $log:= log:log-request("btf","pa")
...
let $login := if (pal:user()) then xmldb:login("/db/apps/btfpa","treeman","fagus") else ()
...
<head>
    <meta charset="UTF-8"/>
    <title>BTF Planning Portal</title>
    <meta name="viewport" content="width=device-width, initial-scale=1"/>
    <link rel="shortcut icon" type="image/png" href="/trees/assets/BTF-new.png"/>
    <link rel="icon" href="/trees/assets/BTF-new.png" sizes="128x128" />

    <link rel="stylesheet" type="text/css" href="/btfpa/assets/screen.css" media="screen" ></link>  
    <link rel="stylesheet" type="text/css" href="/btfpa/assets/mobile.css" media="only screen and (max-device-width: 480px)" ></link> 
    <style type="text/css">
       {$group/style/text()}   
    </style>
    ...
    {if ($context/_signature= ("map","fullmap","application/*")  or $context/_signature="search" and $context/target=("map","fullmap"))
     then ( 
            pa:markers($apps),
            if ($group/area)
            then let $area := doc(concat("/db/apps/btfpa/areas/",$group/area,".xml"))//area
                 return poly:area-polygons($area)
            else (),
           <script type="text/javascript">draggable=false;</script>,
           <script type="text/javascript" src="/btfpa/javascript/map.js"></script> 
          )
      else ()
     }
...
<div>
   ...
   <div class="menu">
     <a href="https://bristoltreeforum.org" target="_blank" ><img src="/trees/assets/BTF-new.png" width="60"/></a>
     <span style="position: absolute; top:5pt;">
